...
```shell
bash deploy_linda.sh --app [FullNode|SolidityNode] --net [mainnet|testnet] --db [keep|remove|backup] 
...
```shell
wget https://raw.githubusercontent.com/lindaprotocol/LindaDeployment/master/deploy_linda.sh -O deploy_linda.sh
bash deploy_linda.sh --app FullNode --net mainnet
```
...
bash deploy_linda.sh --app SolidityNode --net mainnet --trust-node <grpc-ip:grpc-port>
```
...
bash deploy_linda.sh --app SolidityNode --net mainnet --trust-node 127.0.0.1:50051 --rpc-port 50041
```
...
### 概要
本脚本用于部署https://github.com/lindaprotocol/grpc-gateway 代码， 并部署gateway。
### 预安装
请阅读https://github.com/lindaprotocol/grpc-gateway 并安装：go，protoc，请依照个人需求设置好$GOPATH环境变量
### 下载与启动
由于启动脚本可能会发生变化，请每次使用前都重新下载启动脚本
```shell
